 ...
 */
function hashPrefix(fullHash, length = 8) {
 ...
 */
function encryptSnapshot(data, key, keyVersion = 1) {
 ...
 */
function decryptSnapshot(encrypted, getKey) {
...
class AuditLogger {
    ...
    encryptionKey;
    keyVersion;
    constructor(storage, encryptionKey, keyVersion = 1) {
        this.storage = storage;
        this.encryptionKey = encryptionKey;
        this.keyVersion = keyVersion;
    }
    async logResponse(state, results, context, responseText) {
        ...
        const encryptedSnapshot = encryptSnapshot(JSON.stringify(snapshot), this.encryptionKey, this.keyVersion);
    ...
    buildGateAuditEntries(results) {
        const entries = [];
        const gateOrder = ['intent', 'shield', 'lens', 'stance', 'capability', 'model', 'personality', 'spark'];
    ...
    extractTrustViolations(state, results) {
        const violations = [];
