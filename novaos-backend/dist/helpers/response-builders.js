...
exports.buildStoppedResponse = buildStoppedResponse;
exports.buildAwaitAckResponse = buildAwaitAckResponse;
exports.buildDegradedResponse = buildDegradedResponse;
exports.buildResponse = buildResponse;
// ─────────────────────────────────────────────────────────────────────────────────
// INTERNAL HELPERS
// ─────────────────────────────────────────────────────────────────────────────────
function buildTransparency(state, results) {
    return {
...
function buildDebugInfo(results, context, totalMs) {
    const gates = [];
 ...
 */
function sanitizeReasonForUser(reason) {
    if (!reason)
...
// ─────────────────────────────────────────────────────────────────────────────────
function buildStoppedResponse(state, results, context, startTime = Date.now()) {
...
// ─────────────────────────────────────────────────────────────────────────────────
function buildAwaitAckResponse(state, results, context, startTime = Date.now()) {
    const totalMs = Date.now() - startTime;
    if (!state.pendingAck) {
        // Should never happen, but fail safely
        return buildStoppedResponse({ ...state, stoppedAt: 'shield', stoppedReason: 'Missing acknowledgment data' }, results, context, startTime);
    }
    return {
        success: false,
        stopped: false,
...
// ─────────────────────────────────────────────────────────────────────────────────
function buildDegradedResponse(state, results, context, degradeReason, startTime = Date.now()) {
    const totalMs = Date.now() - startTime;
...
// ─────────────────────────────────────────────────────────────────────────────────
function buildResponse(state, results, context, startTime = Date.now()) {
    const totalMs = Date.now() - startTime;
    return {
        success: true,
        message: state.validated?.text ?? state.generation?.text ?? '',
