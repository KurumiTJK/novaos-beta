...
class ModelGate {
    ...
    async execute(state, context) {
        ...
        try {
            ...
            try {
                ...
                const fallbackResponse = await this.invokeFallbackModel(input.message, fallbackConstraints, stance);
                return {
                    gateId: this.gateId,
                    status: 'soft_fail',
                    output: {
    ...
    buildConstraints(state) {
        ...
        if (state.verification?.plan?.verificationStatus === 'skipped') {
            const domain = (0, freshness_checker_js_1.detectDomain)(state.input.message);
            if ((0, freshness_checker_js_1.isImmediateDomain)(domain)) {
        ...
        if (state.risk?.requiredPrependResources) {
            constraints.mustPrepend = (0, safety_renderer_js_1.renderCrisisResourceBlock)();
        }
     ...
     */
    async invokeModel(model, systemPrompt, userMessage, stance) {
        ...
        const baseResponse = stanceResponses[stance] || stanceResponses.lens;
        return {
            text: baseResponse,
     ...
     */
    async invokeFallbackModel(userMessage, constraints, stance) {
        // Simulated fallback
        return {
